...
from skimage.morphology import dilation, disk, square, white_tophat
            ...
            if correction_algo != LSR_Backgound_Correction:
                if i == (z_slice.shape[0]-1):
                    ...
                    if correction_algo == Gaussian_and_Gamma_Correction:
                        ...
                        corrected_channel = correction_algo(channel) # This line will run any non-default (user supplied) background elimination algorithms.
                corrected_channel = np.array(corrected_channel, dtype='uint16')
...
def white_hat(z):
    hot_pixels = white_tophat(z)
    z_norm = z - hot_pixels
    
    z_norm = z_norm.flatten()
    z_norm[list(z_norm<0)] = 0.0
    z_norm = z_norm.reshape(2048,2048)
    return z_norm
    
def Guassian_E(image):
    """Background Correction via Elimination of Scaled Guassian Blur"""
    data_bkg_elim = image-(ndimage.gaussian_filter(image,1.4)*1.1)
    data_bkg_elim = data_bkg_elim - np.min(data_bkg_elim)
    return ndimage.gaussian_filter(data_bkg_elim,0.8)
